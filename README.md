# zegame - A game template for ddframework and tinyecs

tl;dr description here

## Synopsis

```console
one-liner usage example
```

## Usage

Detailed usage and sample code for libraries, full help and options for tools.

## Installation

Installation instructions + requirements (should come automagically with pyproject.toml)

```console
sample install session
```

## Support / Contributing

Issues can be opened on [Github](https://github.com/dickerdackel/zegame/issues)

## License

This software is provided under the MIT license.

See LICENSE file for details.
